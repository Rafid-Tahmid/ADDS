...
public:
    ...
    void insert(T element)
    {
        ...
        while (index > 1 && this->tree.at(index) < this->tree.at(getParentPosition(index)))
        {
    ...
    void remove(T value)
    {
        ...
        {
            heapIndex parentIndex = getParentPosition(indexToRemove);
            if (indexToRemove > 1 && this->tree.at(indexToRemove) < this->tree.at(parentIndex))
            {
                while (indexToRemove > 1 && this->tree.at(indexToRemove) < this->tree.at(getParentPosition(indexToRemove)))
                {
                    parentIndex = getParentPosition(indexToRemove);
                    std::swap(this->tree.at(indexToRemove), this->tree.at(parentIndex));
